...
from .performer_pytorch_opt import PerformerLM, Performer, FastAttention, SelfAttention, CrossAttention, ProjectionUpdater
...
# sys.path.insert(0, './performer_pytorch/')
# from performer_pytorch_opt import SelfAttention
...
class MaskedConv1D(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # print ('The mask computation: ', self.stride, out_mask.shape, out_conv.shape)
...
class MaskedMHCA(nn.Module):
    ...
    def __init__(
        ...
        #performer attention
        self.self_attention_layer = SelfAttention(dim = n_embd, heads = n_head, causal = False)
    ...
    def forward(self, x, mask):
        ...
        out, rep_loss = self.self_attention_layer(torch.transpose(q[:,:,0:qx_mask.nonzero().shape[0]], 2,1))
        out = torch.transpose(out, 2,1)
        out = torch.cat((out, q[:,:,out.shape[2]:q.shape[2]]),2)
