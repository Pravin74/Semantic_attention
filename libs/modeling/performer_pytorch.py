...
def linear_attention(q, k, v):
    ...
    return out, torch.sum(atten_to_viss,0)
...
class FastAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        out, atten_to_viss = attn_fn(q, k, v)
        return out, atten_to_viss
...
class Attention(nn.Module):
    ...
    def forward(self, x, pos_emb = None, context = None, mask = None, context_mask = None, **kwargs):
        ...
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h = h), (q, k, v))   # spliting in number of heads
        (q, lq), (k, lk), (v, lv) = map(lambda t: (t[:, :gh], t[:, gh:]), (q, k, v))

        # print ("after splitting to head:", q.shape, k.shape, v.shape)
        ...
        if not empty(q):
            ...
            out, atten_to_viss = self.fast_attention(q, k, v)
        ...
        out =  self.to_out(out)
        return (self.dropout(out), atten_to_viss)
